...
@section("content")
    ...
    <div class="swcontainer sw-ucenter" style="margin-top: 5%;">
        <!-- 个人中心左侧 -->
                <!-- 个人中心主体 -->
    <!-- 专家视频咨询 / start -->
    <h3 class="main-top" style="font-size: 23px;margin-left: 3%;">在线约见厅</h3>
        ...
        <div class="main-right">
                ...
                <div class="video-manage-top">
                    <div class="vid-man-top-lt vid-man-top-main">
                        <div class="vid-man-top-con">
                            <p class="vid-man-top-cat"><span class="light-color">企业名称：</span>{{$datas->enterprisename}}</p>
                            <p class="vid-man-top-cat"><span class="light-color">金额：</span>{{$datas->price}}</p>
                            <p class="vid-man-top-cat"><span class="light-color">开始时间：</span>{{$datas->puttime}}</p>
                            <p class="vid-man-top-cat"><span class="light-color">会议时长：</span>{{$datas->timelot}}小时</p>
                            <span class="light-color">描述：</span>
                            <div class="vid-man-top-desc">{{$datas->contents}}</div>
                        </div>
                    </div>
                    <div class="vid-man-top-rt vid-man-top-main">
                        <div class="vid-man-top-con">
                            <div class="emcee">
                                ...
                                <span class="emceer-pers"><img src="{{env('ImagePath').$datas->showimage}}" class="vid-new-ava">{{$datas->enterprisename}}</span>
                            </div>
                            <div class="emcee-bottom">
                                <span class="light-color emcee-cap emcee-bot-cap">专家：</span>
                                <div class="emcee-members">
                                    <span class="emceer-pers"><img src="{{env('ImagePath').$expert->showimage}}" class="vid-new-ava">{{$expert->expertname}}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="wrapper box-sizing">
                ...
                <div class="chat-room-btns">
                    <button type="button" class="chat-room-btn goback" onclick="window.location='{{url("/entmymeet/mymeetindex")}}'">返回约见列表</button>
    ...
    <script>
        $(function(){
            ...
            $.ajax({
                url:"{{url('compareMeetTime')}}",
                data:{"meetid":meetid,"nowTime":nowTime},
                dateType:"json",
                type:"POST",
        ...
        $(".chat-room-btn1").on('click',function(){
            ...
            $.ajax({
                ...
                success:function(res){
                    if(res['code']=='success'){
                        layer.msg('会议已完成',function () {
                            window.location="{{url("/entmymeet/myMeetIndex")}}";
                        });
                    }else{
        ...
        var time=setInterval(function(){
            ...
            $.ajax({
                url:"{{url('compareMeetTime')}}",
                data:{"consultId":meetid,"nowTime":nowTime},
                dateType:"json",
                type:"POST",
                success:function(res){
                    if(res['code']=="success"){
