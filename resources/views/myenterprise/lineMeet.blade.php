...
@section("content")
        ...
        <div class="ucenter-con">
            <div class="main-right">
                <div class="card-step works-step">
                    ...
                    <span class="gray-circle">2</span>等待确认<span class="card-step-cap">&gt;</span>
                    <span class="gray-circle">3</span>已经确认<span class="card-step-cap">&gt;</span>
                    <span class="gray-circle">4</span>线下约谈
                </div>
                <div class="invite-experts apply-video-wrapper">
                    <table class="invite-table">
                        <tr>
                            <td>专家姓名</td>
                            <td>
                                <ul class="uct-linemeet-expertname"></ul>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td>收费标准</td>
                            <td>
                                <ul class="uct-linemeet-linefee"></ul>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td>专家头像</td>
                            <td>
                                <ul class="uct-works-expava"></ul>
                            </td>
                        </tr>
                        <tr>
                            <td>问题描述</td>
                            <td>
                                <textarea name="" class="publish-need-txt uct-works-txt" cols="30" rows="10" placeholder="请输入问题描述"></textarea>
                            </td>
                        </tr>
                        <tr>
                            <td>约见时间</td>
                                   ...
                                   {{-- <div class="calendar-start clearfix">
                                    ...
                                    <div class="calendar-end clearfix">
                                        ...
                                        <div class="modals publish-need-sel" style="margin-left:15px;width: 237px">
                                            <a href="javascript:;" class="modal-choose" id="time">1</a>
                                        ...
                                        <span style="margin-left: 16px;float: right;">小时</span>
                                    </div>
                                </div>
                            </td>
                        </tr>
                    ...
                    <div class="uct-works-con">
                        <button class="test-btn submit-audit" type="button">确认约见</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
        ...
        $(function(){
            ...
            if($.cookie("videoreselect")){
                ...
                for(var i=0; i<expertChecked.length; i++) {
                    ...
                    var expertname=checked.substring(0,end);

                    var checked=checked.substring(end+1);
                    var end=checked.indexOf("/");
                    var linefee=checked.substring(0,end);

                    var checked=checked.substring(end+1);
                    var end=checked.indexOf("/");
                    var id=checked.substring(0,end);

                    var checked=checked.substring(end);
                    var end=checked.indexOf("/");id
                    var img=checked.substring(end);
                    //alert(end)

                    var str = expertname
                    $(".uct-linemeet-expertname").append(str);

                    var str = '￥'+linefee+'/小时'+"<input type='hidden' name='linefee' value="+linefee+">"
                    $(".uct-linemeet-linefee").append(str);
        ...
        $(".submit-audit").on("click",function(){
         ...
         if({{$enterpriseid}}!=3){
                layer.confirm('您的认证企业信息未认证？', {
                        btn: ['去认证','先缓缓'], //按钮
                        skin:'layui-layer-molv'
                    }, function(){
                        window.location.href='/uct_member';
                    }, function(){
                        layer.close();
                    });
                return false;
            ...
            $.ajax({
                url:"{{asset('startMeet')}}",
                data:{"describe":describe,'expertid':expertid,'linefee':linefee,'time':time},
                dateType:"json",
                type:"POST",
                success:function(res){
                    if(res['msg']=='success'){
                        window.location= "{{'/uct_linemeet/detail'}}/"+res['linemeetid'];
                    }else{
                        layer.msg('约见错误',{'icon':5});
