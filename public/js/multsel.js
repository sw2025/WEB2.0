/**
 * multiple select plugin
 * @date: 2017/09/13
 * @author: liugl@inspur.com
 */
function multselInit(obj){
	$_sel=obj;
	var obj2 = obj;
	$_sel.each(function(){
		$(this).append("<i></i>");
	});
	$_sel.find("span.view").click(function(){
		$_t=$(this).parent("div.multsel");
		if($_t.hasClass('expand')){
			$_t.find(".selist").hide();
			$_t.find("i:first").removeClass("pop");
			$_t.removeClass('expand');
		}else {
			$_t.find(".selist").show();
			$_t.find("i:first").addClass("pop");
			$_t.addClass('expand');
		}
	});
	$_sel.find(".selist").find("a.seitem").click(function(){
		$_this=$(this);

		if($_this.hasClass('checked')){
			$_this.removeClass('checked');
		}else{
			$_this.addClass('checked');
		}
		multselCheck($_this.parent(".selist"),obj2);
	});

	$(document).click(function(e){
		var t = $("div.multsel")[0],target = e.target;
		if (t !== target && !$.contains(t, target)) {
			$_sel.removeClass('expand').find(".selist").hide();
			$_sel.find("i:first").removeClass("pop");
		}
	});
}
function multselCheck($obj,$obj2){
	var names='',vals=[];
	$obj.find('a.checked').each(function(){
		var $t=$(this);
		names+=$t.html()+',';
		vals.push($t.attr("value"));
	});
	var domain = $obj2.children('.view').attr('index');
	if($.trim(domain)=='domain'){
		if(names.indexOf('风险投资')){
			$('#domain2').css('display','none');
			$('#preference').css('display','none');
		} else{
			$('#domain2').css('display','block');
			$('#preference').css('display','block');
		}
	}

	if(!names){
		names='请选择';
	}
	$obj.parent('div.multsel').find('span.view').html(names);
	multselVals(vals);
}
/**
	*获得value值
 * interface
 * @param vals
 */
function multselVals(vals){
	var str='';
	for(var i=0,size=vals.length;i<size;i++){
		str+=vals[i]+',';
	}
	$('#testarea').html(str);
}
