$(function(){
    ...
    var movingDistance      = 322;//每次移动的距离
    var curWidth            = 466;//当前中间li的宽度为350px
    var curHeight           =290;//当前中间li的高度未312  
    var curImgWidth         =466;
    var curImgHeight        =290;
    var othersW             =322;//其他li的宽度
    var othersH             =210;//高度
    var othersImgW          =322;//其他li的宽度
    var othersImgH          =210;//高度
    var $panels             = $('#slider .scrollContainer > li');//此处作用是将复制进来补白的元素重新赋值
    var $container          = $('#slider .scrollContainer');
    $panels.css({'float' : 'left','position' : 'relative'});
    $("#slider").data("currentlyMoving", false);//是否正在运动中
    $container.css('width', (($panels[0].offsetWidth) * $panels.length) + 144 ).css('left','0');//计算容器的总的宽度 PS：25为margin值 offsetWidth
